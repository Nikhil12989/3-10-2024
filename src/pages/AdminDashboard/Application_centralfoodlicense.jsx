...
const Application_centralfoodlicense = () => {
  ...
  useEffect(() => {
    const fetchLicenseData = async () => {
      try {
        const response = await axios.get(`http://192.168.1.50:5000/api/centralFoodLicense/getCentralFoodLicense/${id}`);
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto mt-5 mb-10 bg-gray-200 rounded-lg shadow-lg">
      <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-gray-800 mb-8">
        Central Food License Application
      </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
        ...
        <div className="bg-white shadow-md p-4 sm:p-6 rounded-lg space-y-4">
          ...
          {[
            { label: 'Owner Full Name', value: licenseData.fullName },
            { label: 'License Required Years', value: licenseData.licenseRequireYears },
            { label: 'Business Name', value: licenseData.businessName },
            { label: 'Nature of Business', value: licenseData.natureOfBusiness }
          ].map(({ label, value }) => (
            <div key={label} className="flex justify-between text-gray-800">
              <strong className="text-gray-600 w-1/2">{label}:</strong>
              <span className="w-3/4 text-gray-700 border border-gray-300 bg-gray-50 p-2 rounded">{value}</span> {/* Increased width */}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white shadow-md p-4 sm:p-6 rounded-lg space-y-4">
          ...
          {[
            { label: 'Owner Qualification', value: licenseData.ownerQualification },
            { label: 'Mobile Number', value: licenseData.mobileNumber },
            { label: 'Email', value: licenseData.email },
            { label: 'Business Address', value: licenseData.businessAddress }
          ].map(({ label, value }) => (
            <div key={label} className="flex justify-between text-gray-800">
              <strong className="text-gray-600 w-1/2">{label}:</strong>
              <span className="w-3/4 text-gray-700 border border-gray-300 bg-gray-50 p-2 rounded">{value}</span> {/* Increased width */}
